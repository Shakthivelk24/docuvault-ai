...
function toClientDocument(document) {
  return {
    ...
    type:
      document.contentType?.split("/").pop() ||
      document.type ||
      "file",
...
export const getPreviewUrl = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to create preview URL",
    });
  }
};
...
export const completeUpload = async (req, res) => {
  try {
    ...
    await createNotification({
      ...
      body: `${document.fileName} has been uploaded and queued for AI processing.`,
      icon: "FileCheck",
      tint: "text-brand-500",
      bg: "bg-brand-500/10",
      documentId,
    ...
    return res.status(200).json(
      toClientDocument(updatedDocument)
    );
  } catch (error) {
    console.error(
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "At least one field (name or favorite) is required",
      });
    }
